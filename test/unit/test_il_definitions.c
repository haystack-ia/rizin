...
static bool is_euqal_bv(RzILBitVector x, RzILBitVector y) {
	bool equal = (rz_il_bv_cmp(x, y) == 0 ? true : false);
	return equal;
}
...
bool test_rzil_bv_init(void) {
	...
	rz_il_bv_set(bits_cmp, 25, true);
	rz_il_bv_set(bits_cmp, 26, true);
	rz_il_bv_set(bits_cmp, 29, true);
	mu_assert("new from ut32", is_euqal_bv(bits_32, bits_cmp));

	// dup
	RzILBitVector bits_32_dump = rz_il_bv_dump(bits_32);
	mu_assert("dump from bits32", is_euqal_bv(bits_32_dump, bits_32));
...
bool test_rzil_bv_logic(void) {
	...
	rz_il_bv_set(y, 4, true);
	rz_il_bv_set(y, 7, true);

	// and : 0000 0001
	and = rz_il_bv_new0(8);
	rz_il_bv_set(and, 7, true);

	// xor : 1111 1100
	xor = rz_il_bv_new0(8);
	rz_il_bv_toggle_all(xor);
	rz_il_bv_set(xor, 6, false);
	rz_il_bv_set(xor, 7, false);

	// or : 1111 1101
	or = rz_il_bv_new0(8);
	rz_il_bv_toggle_all(or);
	rz_il_bv_set(or, 6, false);

	// not of x : 1010 1010
	not = rz_il_bv_new0(8);
	rz_il_bv_set(not, 0, true);
	rz_il_bv_set(not, 2, true);
	rz_il_bv_set(not, 4, true);
	rz_il_bv_set(not, 6, true);

	// neg of x : 1010 1011
	neg = rz_il_bv_new0(8);
	rz_il_bv_set(neg, 0, true);
	rz_il_bv_set(neg, 2, true);
	rz_il_bv_set(neg, 4, true);
	rz_il_bv_set(neg, 6, true);
	rz_il_bv_set(neg, 7, true);

	// left shift (3 bits) of y : 0100 1000
	ls = rz_il_bv_new0(8);
	rz_il_bv_set(ls, 1, true);
	rz_il_bv_set(ls, 4, true);

	// left shift (3 bits) of y : 0100 1111
	ls_fill = rz_il_bv_new0(8);
	rz_il_bv_set(ls_fill, 1, true);
	rz_il_bv_set(ls_fill, 4, true);
	rz_il_bv_set(ls_fill, 5, true);
	rz_il_bv_set(ls_fill, 6, true);
	rz_il_bv_set(ls_fill, 7, true);

	// right shift (3 bits) of y : 0001 0101
	rs = rz_il_bv_new0(8);
	rz_il_bv_set(rs, 3, true);
	rz_il_bv_set(rs, 5, true);
	rz_il_bv_set(rs, 7, true);

	// right shift (3 bits) of y : 1111 0101
	rs_fill = rz_il_bv_new0(8);
	rz_il_bv_toggle_all(rs_fill);
	rz_il_bv_set(rs_fill, 4, false);
	rz_il_bv_set(rs_fill, 6, false);

	// test and
	result = rz_il_bv_and(x, y);
	mu_assert("and x y", is_euqal_bv(result, and));
	rz_il_bv_free(result);

	result = rz_il_bv_or(x, y);
	printf("\n");
	mu_assert("or x y", is_euqal_bv(result, or));
	rz_il_bv_free(result);

	result = rz_il_bv_xor(x, y);
	mu_assert("xor x y", is_euqal_bv(result, xor));
	rz_il_bv_free(result);

	result = rz_il_bv_not(x);
	mu_assert("not x", is_euqal_bv(result, not ));
	rz_il_bv_free(result);

	result = rz_il_bv_neg(x);
	mu_assert("neg x", is_euqal_bv(result, neg));
	rz_il_bv_free(result);

	result = rz_il_bv_dump(y);
	rz_il_bv_lshift(result, 3);
	mu_assert("left shift y", is_euqal_bv(result, ls));
	rz_il_bv_free(result);

	result = rz_il_bv_dump(y);
	rz_il_bv_lshift_fill(result, 3, true);
	mu_assert("left shift y filling 1", is_euqal_bv(result, ls_fill));
	rz_il_bv_free(result);

	result = rz_il_bv_dump(y);
	rz_il_bv_rshift(result, 3);
	mu_assert("right shift y", is_euqal_bv(result, rs));
	rz_il_bv_free(result);

	result = rz_il_bv_dump(y);
	rz_il_bv_rshift_fill(result, 3, true);
	mu_assert("right shift y", is_euqal_bv(result, rs_fill));
	rz_il_bv_free(result);
...
bool test_rzil_bv_operation(void) {
	...
	rz_il_bv_set(prep, 7, true);

	// append 5 : 0000 1000 0000 0
	append = rz_il_bv_new0(13);
	rz_il_bv_set(append, 4, true);

	// cut head 2: 00 1000
	cut_h = rz_il_bv_new0(6);
	rz_il_bv_set(cut_h, 2, true);

	// cut tail 4: 0000
	cut_t = rz_il_bv_new0(4);

	res = rz_il_bv_prepend_zero(x, 3);
	mu_assert("prepend 3 zero", is_euqal_bv(res, prep));
	rz_il_bv_free(res);

	res = rz_il_bv_append_zero(x, 5);
	mu_assert("append 5 zero", is_euqal_bv(res, append));
	rz_il_bv_free(res);

	res = rz_il_bv_cut_head(x, 2);
	mu_assert("cut head 2 zero", is_euqal_bv(res, cut_h));
	rz_il_bv_free(res);

	res = rz_il_bv_cut_tail(x, 4);
	mu_assert("cut tail 4 zero", is_euqal_bv(res, cut_t));
	rz_il_bv_free(res);
